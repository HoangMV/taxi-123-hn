...
module.exports = async function handler(request, response) {
  ...
  try {
    const idHoSoDaoTao = cleanValue(
      request.query?.ID_HoSoDaoTao ||
      request.query?.idHoSoDaoTao ||
      request.body?.ID_HoSoDaoTao ||
      request.body?.idHoSoDaoTao
    );
    ...
    const providedRow =
      ...
      cleanValue(request.body.row.ID_HoSoDaoTao) === idHoSoDaoTao
        ? request.body.row
        : null;
    ...
    const donViPromise = findRowsByIds(config, 'DONVI', 'ID_DonVi', [row.Ref_DonViDeNghi]);
