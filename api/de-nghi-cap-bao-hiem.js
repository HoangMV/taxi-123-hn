...
module.exports = async function handler(request, response) {
  ...
  try {
    const idHoSoBaoHiem = cleanValue(
      request.query?.ID_HoSoBaoHiem ||
      request.query?.idHoSoBaoHiem ||
      request.body?.ID_HoSoBaoHiem ||
      request.body?.idHoSoBaoHiem
    );
    ...
    const providedRow =
      ...
      cleanValue(request.body.row.ID_HoSoBaoHiem) === idHoSoBaoHiem
        ? request.body.row
        : null;
    ...
    const congTyBaoHiemPromise = findRowsByIds(config, 'DM_CTY_BAOHIEM', 'ID_CongTyBaoHiem', [row.Ref_CongTyBaoHiem]);
